    ...
    return (
        <>
            <div className="overflow-y-auto hover:bg-royalblue-300 p-2 m-1 flex-wrap flex items-center gap-2  rounded-md bg-background ">
                {user.profilePicture ? <img className='rounded' alt={`${user.displayName}'s Profile Picture`} src={`https://disclone.duckdns.org${user.profilePicture}`} /> : <img className='rounded' src={defaultProfilePicture} alt={`${user.displayName}'s Profile Picture`} />}
